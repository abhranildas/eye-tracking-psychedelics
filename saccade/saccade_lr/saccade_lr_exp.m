...
try
    ...
    [window,windowrect]=Screen('OpenWindow', screenNumber, 0,[],32,2); %#ok<*NASGU>
    ...
    % fixation dot
    fix_pos([1 3]) = winWidth/2+dot_radius*[-1 1];
    fix_pos([2 4]) = winHeight/2+dot_radius*[-1 1];
    ...
    for iTrial=1:nTrials
        ...
        x_target=winWidth/2*(target_dir(iTrial)+.5);
        y_target=winHeight/2;

        target_pos([1 3]) = x_target+dot_radius*[-1 1];
        target_pos([2 4]) = y_target+dot_radius*[-1 1];

        x_antitarget=winWidth/2*(1.5-target_dir(iTrial));
