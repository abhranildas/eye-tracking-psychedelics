function saccade_lr_exp
% SACCADE_LR_EXP  Run one session of the left/right pro-saccade / anti-saccade task.
%   saccade_lr_exp
%
...
%   setup_lr.mat in the current directory. No script in this repo produces
%   that file (bug 0.4.21 below); it exists only as a shipped artifact.
%
...
%   fixation dot for fixation_duration, then a peripheral target to the left or
%   right of screen centre, at target_dir(i_trial), shown for target_duration
%   together with a dim grey anti-target on the opposite side. A green target
%   means pro-saccade (look at it) and a red one means anti-saccade (look at
%   the grey dot instead); for a colour-blind subject the red dot is replaced
%   by a red cross of the same size, since colour alone would not carry the
%   instruction. Finally the recording is closed, transferred off the Host PC
%   and renamed.
%
...
%
%   setup_lr.mat  loaded from the current directory, so run this file with
%              saccade/saccade_lr/ as the working directory. Two variables,
%              both frozen by the shipped artifact (docs/repo-cleanup.md
%              section 4.1.1, zone 3), so neither the load string nor the
%              variable names may change:
%                target_dir    [n_trials x 1] logical, DIMENSIONLESS. Which
%                              side of screen centre the target appears on;
%                              not an angle, unlike saccade_exp's target_ang,
%                              which is frozen in a different artifact
%                              (saccade/setup.mat) -- that difference is why
%                              this file and saccade_exp.m cannot be merged.
...
%   0.4.21  No file among this repo's 18 in-scope .m files generates or
%              regenerates saccade/saccade_lr/setup_lr.mat, unlike
%              saccade/setup.mat, which saccade_setup.m writes (with its own
%              bug, 0.4.4). If setup_lr.mat is ever lost there is no script
%              here to recreate it.
%
% See also SACCADE_EXP, SPEM_EXP, FIXATE_EXP, FREE_VIEW_EXP,
%   LIB.PREPROCESS_GAZE, EYELINKINITDEFAULTS

% set up trials
load('setup_lr.mat', 'target_dir', 'saccade_type');
n_trials = size(target_dir, 1);
...
try
    ...
    for i_trial = 1:n_trials
        ...
        % on the opposite side of screen centre from the target.
        x_target = win_width_px/2*(target_dir(i_trial) + .5);
        y_target = win_height_px/2;

        target_pos([1 3]) = x_target + dot_radius*[-1 1];
        target_pos([2 4]) = y_target + dot_radius*[-1 1];

        x_antitarget = win_width_px/2*(1.5 - target_dir(i_trial));
        antitarget_pos([1 3]) = x_antitarget + dot_radius/2*[-1 1];
        antitarget_pos([2 4]) = y_target + dot_radius/2*[-1 1];
        ...
        % NOTE: bug 0.4.19 (documented in saccade_exp.m) applies to this
        % file too -- target_colour is assigned three times below and never
        % read; the FillOval and DrawLine calls all use literal colour
        % vectors instead. Left in place deliberately.
        ...
        % target_dir is a logical, saccade_type is a logical printed as a
        % float. Both names match the frozen setup_lr.mat variables, so the
        % message text must not be reworded.
        msg_target_dir = sprintf('!V TRIAL_VAR target_dir %d ', target_dir(i_trial));
        msg_saccade_type = sprintf('!V TRIAL_VAR saccade_type %2.3f ', saccade_type(i_trial));
        Eyelink('Message', msg_target_dir);
