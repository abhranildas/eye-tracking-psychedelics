...
nTrials=size(target_ang,1);
fixation_duration=1; % in sec
target_duration = 1; % in sec
dot_radius=5;
...
try
    ...
    target_circ_radius=winWidth/4; % radius of the circle around which target will appear

    %     moviePtr = Screen('CreateMovie', window, 'saccade_movie.mp4', [],[],1);

    % fixation dot
    fix_center=[winWidth winHeight]/2;
    ...
    half_length   = 8;        % length in each direction from center
    lineWidth     = 4;         % thickness of the lines

    % Calculate the end coordinates of the two diagonals:
    ...
    for iTrial=1:nTrials
        ...
        % get target and anti-target position
        x_target=fix_center(1)+target_circ_radius*cos(target_ang(iTrial));
        y_target=fix_center(2)+target_circ_radius*sin(target_ang(iTrial));

        target_pos([1 3]) = x_target+dot_radius*[-1 1];
        target_pos([2 4]) = y_target+dot_radius*[-1 1];

        x_antitarget=fix_center(1)-target_circ_radius*cos(target_ang(iTrial));
        y_antitarget=fix_center(2)-target_circ_radius*sin(target_ang(iTrial));
        ...
        if saccade_type(iTrial) % if pro-saccade
        ...
        %         EyelinkDoDriftCorrection(el,round(winWidth/2),round(winHeight/2));
        ...
        if saccade_type(iTrial) % if pro-saccade
            ...
            elseif strcmpi(color_blind,'y')
                % show red cross
                Screen('DrawLine', window, [255 0 0], x_target+x1_1, y_target+y1_1, x_target+x1_2, y_target+y1_2, lineWidth);
                Screen('DrawLine', window, [255 0 0], x_target+x2_1, y_target+y2_1, x_target+x2_2, y_target+y2_2, lineWidth);
            end
        end
        ...
        %         imwrite(screenshot, sprintf('screenshot_%d.png',iTrial));
        ...
        msg1 = sprintf('!V TRIAL_VAR target_ang %.1f ', target_ang(iTrial));
