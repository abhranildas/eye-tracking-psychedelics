...
elseif exist(['data/' edf_file '.edf'],'file')
    ...
    type=regexp(edf_file, pattern, 'tokens');
    type=type{1}{1};
end

% set up trials
load(['setup/setup_' type '.mat'],'exp_IAPS_ids');

nTrials=size(exp_IAPS_ids,1);
n_pics=size(exp_IAPS_ids,2);
fixation_duration=0.2; % in sec
target_duration = 10; % in sec
dot_radius=5;
...
try
    ...
    [window,rect]=Screen('OpenWindow', screenNumber, 0,[],32,2); %#ok<*NASGU>
    ...
    for iTrial=1:nTrials
        ...
        % Load the images
        img_nums = exp_IAPS_ids(iTrial,:);
        imageTextures = cell(1, n_pics);
        for i = 1:n_pics
            imageData = imread(['data/IAPS/' num2str(img_nums(i)) '.jpg']);
            imageTextures{i} = Screen('MakeTexture', window, imageData);
        end
        ...
        numCols = 3;
        numRows = 3;
        quadWidth = screenWidth / numCols;
        quadHeight = screenHeight / numRows;
        quads = cell(numRows, numCols);
        for r = 1:numRows
            for c = 1:numCols
                quads{r, c} = [(c - 1) * quadWidth, (r - 1) * quadHeight, c * quadWidth, r * quadHeight];
            end
        end
        
        % Display the images in quadrants
        for r = 1:numRows
            for c = 1:numCols
                Screen('DrawTexture', window, imageTextures{(r - 1) * numCols + c}, [], quads{r, c});
            end
        end
